...
说法（对照表见 concept-method.md「互动叙事这条线」，先例名单见 intake-benchmark-reference.md
第 1.5 节）。反过来，选了叙事主导也不得省略结构选型：主干加瓶颈 / 任务簇 / 长廊 /
分院帽 / 树洞，选一个并说明理由。
...
- 三段弧：探索期在发现什么、成长期什么在复利、成熟期玩家做得到什么新手做不到的事——
  每期必须改变玩家**做的事**（新动词 / 新可达空间），只有数字变大不算。叙事主导按同样
  强度判，差集换成：新的可问对象与话题、新的质证或施压手段、因先前行为而改变的人物
  态度、只有知情者才走得到的场景与结局；**多一段文本、或多一个不改变后续的对白选项，
  与"数字变大"同样不算差集**；
...
1. 一页产品定义，含选定方向的目标语言与文化市场，以及选定的 `experienceProfile`
   （`system-led` / `narrative-led` / `hybrid`）——它继承自 `PRODUCT_BRIEF`，本阶段确认或
   提出修订，并决定下游读哪几份方法文件；叙事主导还要写明所选分支结构（主干加瓶颈 /
   任务簇 / 长廊 / 分院帽 / 树洞）；
