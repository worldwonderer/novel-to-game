...
`CONCEPT.md` 的 `experienceProfile` 为 `narrative-led` 或 `hybrid` 时，在核心六项之外补以下
断言。它们都是确定性检查，可自动断言：

- **分支可达**：GAME_DESIGN 声明的每条主要路径与每个结局都能从 clean start 实际走到；
  走不到的写 FAIL，不写"内容未完成"。
- **旗标被消费**：每个持久旗标存在至少一个实际读取点，且该读取点在某条路径上真的触发。
  只写不读的旗标按缺陷报告，归属 design 或 build。
- **未选事实不串线**：对关键分支走正向与反向两条路径，断言未选择的行为没有出现在后续
  文本、人物台词或可用行动里。这是"人物说明逻辑混乱"最常见的成因。
- **人物知识边界**：人物在某场使用的信息，必须是他在该场之前已经获得的。用知识表逐条
  对照，越界即 FAIL。
- **回响存在**：GAME_DESIGN 声明的每处延迟回响，在满足条件的路径上确实出现在玩家可见
  文本里，且点名玩家做过的具体事。
- **结局区分**：两条做出不同关键选择的路径，不得到达字字相同的结局文本。
