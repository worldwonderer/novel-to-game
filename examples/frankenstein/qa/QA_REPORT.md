...
- Date: 2026-07-28
- Round: **sixth**. Round 1 was the first independent QA (before it the example had no `qa/`
  ...
  of held input with a frozen picture (F17/F18/F19, all **fixed**).
- Verdict: **not `PASS`** — the slice is **completable by either input scheme**: full eight-night
  ...
  executable evidence**. **All release gates now close.** The verdict is still not `PASS` because the two
  human judgements remain — clean-context onboarding comprehension and a playtest — plus the
  signature-frame pass and F4/F13 (see *Findings*, *Not verified*).
- `blocker`: 0 (1 found in round 1, **fixed**, with a regression check)
- `major`: 6 — 1 partly cleared (images and fonts done; audio outstanding), 3 **fixed**
  (F6 mouse input, F7 the `seen` ending, F12 the hovel plate's composition)
- `minor`: 11 fixed, 3 open
...
| Browser assertions | **337 passed, 0 failed** across nine passes: keyboard, mouse, keyboard campaign, mouse campaign, and all four designed endings (`door`, `seen`, `want`, `silence`), plus a card-layout gate over 2898 layouts and a font gate proving both Caslon faces really render |
...
Machine-readable summary: `qa/evidence/automated.json`. Frames: `qa/evidence/browser/` (47).
...
**Pacing evidence, measured at normal speed on the finished build (2026-07-29):** frame
p50 8.3 / p95 10.1 / worst 10.4 ms against the 30 FPS floor of 33.4 ms; **no stall over 200 ms and
no main-thread block over 50 ms in game**. Both full campaigns complete eight nights to the `door`
ending at normal speed. These numbers are what queue item 13 was for and they stand. What does
**not** yet hold is a clean suite at normal speed — see F16, where two of the newest passes fail.

**Earlier note, superseded:** Round 1's figures came from an accelerated run and were not usable for pacing. The
