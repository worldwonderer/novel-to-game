...
| Browser assertions | **291 passed, 0 failed** across nine passes: keyboard, mouse, keyboard campaign, mouse campaign, and all four designed endings (`door`, `seen`, `want`, `silence`), plus a card-layout gate over 2898 layouts and a font gate proving both Caslon faces really render |
...
| F14 | `minor` | `qa` | **The position-filter assertion is weaker than it looks.** `[audio]` asserts the yard hearing is more open than the hovel's by comparing broadband RMS, and it passes — 0.02186 vs 0.02139. But that is a **2.2% margin**, because the hovel combines a 900 Hz low-pass (attenuating) with a reflection gain of 0.16 against the yard's 0.07 (adding). The two effects nearly cancel, so the test measures their net rather than the filter, and a modest change to either reflection gain could flip its sign | **Open** — the layer itself is correct and the assertion is directionally right; the gate is the weak part. A robust version would compare energy **above 900 Hz** between the two positions rather than broadband RMS. Filed now, while only one cue exists, because item 12 adds eleven more that will inherit the same stage |
...
| Release-gated audio set | **1 of 12 present** — `src/audio.js` exists, mirroring `skin.js`: cues enter by key, a missing key is silent and reports as pending, the run continues. `latch` is authored and fires from engine state; the other eleven are honestly still in `gatedPending()`. Procedural WebAudio per §13.4 ("short filtered noise bursts with authored envelopes") — no audio files. §13.1's position stage is in place (hovel low-pass 900 Hz + reflection / yard open / room dry) |
