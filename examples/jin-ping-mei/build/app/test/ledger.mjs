...
section('身份与存档');
test('新局版本 9 且三条成人深线不变',()=>{const s=E.newGame(1);eq(s.version,9);eq(Object.keys(s.relations),['wu_yueniang','pan_jinlian','li_pinger']);eq(s.visits,{wu_yueniang:0,pan_jinlian:0,li_pinger:0});eq(s.accords,{order:false,truth:false,safety:false});eq(s.jointActions,[])});
...
test('v3 存档补齐宅中人后迁入 v9',()=>{const old=E.newGame(1);old.version=3;delete old.household;delete old.currentHouseholdEvent;delete old.publicOverrides;delete old.routeReopensOn;delete old.visits;delete old.accords;delete old.sharedNightChoice;delete old.jointActions;delete old.currentJointAction;const loaded=E.deserialize(JSON.stringify(old));eq(loaded.version,9);eq(Object.keys(loaded.household),HOUSEHOLD_IDS);eq(Object.keys(loaded.publicOverrides),['wu_yueniang','pan_jinlian','li_pinger']);eq(loaded.visits,{wu_yueniang:0,pan_jinlian:0,li_pinger:0});eq(loaded.accords,{order:false,truth:false,safety:false});eq(loaded.jointActions,[])});
test('v4 存档按失信旗标反推越过计数后迁入 v9',()=>{const old=E.newGame(1);old.version=4;old.flags={broken_pan_word:true};delete old.publicOverrides;delete old.routeReopensOn;delete old.visits;delete old.accords;delete old.sharedNightChoice;delete old.jointActions;delete old.currentJointAction;const loaded=E.deserialize(JSON.stringify(old));eq(loaded.version,9);eq(loaded.publicOverrides.pan_jinlian,1);eq(loaded.publicOverrides.wu_yueniang,0);ok(!E.routeCooling(loaded,'pan_jinlian'))});
test('v5 存档只按已结算 visit_choice 回填拜访次数',()=>{const old=E.newGame(1);old.version=5;delete old.visits;delete old.accords;delete old.sharedNightChoice;delete old.jointActions;delete old.currentJointAction;old.history=[{day:1,type:'visit_start',heroine:'li_pinger'},{day:1,type:'visit_choice',heroine:'li_pinger'},{day:2,type:'visit_start',heroine:'li_pinger'},{day:3,type:'visit_start',heroine:'wu_yueniang'}];const loaded=E.deserialize(JSON.stringify(old));eq(loaded.version,9);eq(loaded.visits,{wu_yueniang:0,pan_jinlian:0,li_pinger:1})});
...
test('v5 无历史存档 visits 置 0 重入不崩',()=>{const old=E.newGame(1);old.version=5;delete old.visits;delete old.accords;delete old.sharedNightChoice;delete old.jointActions;delete old.currentJointAction;old.history=[];const loaded=E.deserialize(JSON.stringify(old));eq(loaded.version,9);eq(loaded.visits,{wu_yueniang:0,pan_jinlian:0,li_pinger:0});loaded.phase='choose_visit';ok(E.visitChoices(loaded,'pan_jinlian').length>0,'置 0 后从第 1 拍重入,有选项可列')});
test('v6 在途晨间存档补空报条迁入 v9,已结转的账不重扣',()=>{const old=E.newGame(1);old.version=6;delete old.accords;delete old.sharedNightChoice;delete old.jointActions;delete old.currentJointAction;old.day=3;old.phase='morning';old.morning={id:'quiet',actor:'li_pinger',tone:'quiet',title:'天亮了',text:'茶'};const silver=old.resources.silver;const loaded=E.deserialize(JSON.stringify(old));eq(loaded.version,9);eq(loaded.morning.notes,[]);eq(loaded.resources.silver,silver,'迁移不得重扣用度或催账')});
test('v7 存档从 0/3 共约继续,不凭旧数值直送新结局',()=>{const old=E.newGame(1);old.version=7;delete old.accords;delete old.sharedNightChoice;delete old.jointActions;delete old.currentJointAction;for(const id of HEROINE_IDS){old.relations[id].qing=80;old.relations[id].du=0}old.flags.banquet_balanced=true;const loaded=E.deserialize(JSON.stringify(old));eq(loaded.version,9);eq(loaded.accords,{order:false,truth:false,safety:false});eq(loaded.jointActions,[]);eq(E.determineEnding(loaded).id,'unstable')});
test('v8 存档保留院约但从 0/2 联院差事继续',()=>{const old=E.newGame(1);old.version=8;old.accords={order:true,truth:true,safety:true};delete old.jointActions;delete old.currentJointAction;const loaded=E.deserialize(JSON.stringify(old));eq(loaded.version,9);eq(loaded.accords,{order:true,truth:true,safety:true});eq(loaded.jointActions,[]);eq(loaded.currentJointAction,null)});
