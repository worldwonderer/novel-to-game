...
机器事实源：`qa/verification.json`。静态/规则汇总：`qa/evidence/automated.json`。快速与常速浏览器
记录分别写入 `qa/evidence/browser/evidence-fast.json` 与 `evidence-normal.json`。逐步截图按需重建，
...
| 检查 | 结果 | 证据 |
|---|---|---|
| 启动、渲染和输入 | PASS | `qa/evidence/automated.json`、浏览器记录 |
| 六日核心循环与结果 | PASS | 同上 |
| 延迟后果与群体冲突 | PASS | 同上 |
| 重开与场景册 | PASS | 同上 |
...
- `publicationTier` 仍为 `graybox`；公开托管记录未绑定当前 release fingerprint，阻断 `release`。
